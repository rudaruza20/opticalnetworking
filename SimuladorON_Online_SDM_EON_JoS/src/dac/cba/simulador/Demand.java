...
public class Demand {
	...
	private double bw; // analogue BW in GHz // 25062022
		...
		if (t_baudrate=="fixed"){
			for (int i=0;i<d_r.size();i++){
				if (probability <= intervals.get(i)){
					this.bitRate = d_r.get(i); //15022025. Bit-rate in terms of bps
	...
	//new 15022025
	public Demand (int srcid, int dstid, double bitrate, Network net){
		this.id=uniqueId.getAndIncrement();
		this.srcnode = net.GetNode(srcid);
		this.dstnode = net.GetNode(dstid);
		this.bitRate = bitrate;
