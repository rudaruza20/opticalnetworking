...
/*
 *Joint implementation of MIN number of MIMO LPs and MIN Spectrum Occupation
 */
...
public class EON_SDM_ILP9_MIMO {
					...
					for (Path p: net.GetPaths(d.GetSrcNode().GetId(), d.GetDstNode().GetId(), pos-2)){
						for (Channel c:p.getChannels()){
							if(cplex.getValue(x[l])>=0.99 && cplex.getValue(x[l])<=1.01){
								System.out.print("Demand "+d.GetId()+" utilizes candidate path "+j+" with ");//+p.GetPath().get(0).GetSrcNode().GetId());
